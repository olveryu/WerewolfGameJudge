...
# @game-judge/api-worker Standards
...
- Use `jsonBody(schema)` middleware (`platform/http/jsonBody.ts`) to validate JSON bodies.
...
- Schemas stay with their owner under `features/*`, `games/*`, or `platform/*`; there is no shared root schema directory.
...
- `requireAuth` (`features/auth/tokenAuth.ts`, `createMiddleware` from `hono/factory`) authenticates the current D1 principal and sets `c.var.userId` plus `c.var.isAnonymous`.
- Optional auth uses `extractBearerToken` plus `authenticateAccessToken`; there is no signature-only authentication entry point.
...
| `readCloudflareRequestMetadata(request)` | `platform/http/requestMetadata.ts`   | Runtime parsing of consumed `Request.cf`    |
...
- Draw/exchange use the D1 mutation ledger: global idempotency claim, history, stats OCC update, replay response, and loser cleanup commit in one `DB.batch()` transaction.
- Daily reward uses insert-winner semantics for the first stats row and versioned updates for existing rows; concurrent requests award exactly once.
- `crypto.getRandomValues()` for random number generation; probability functions (`rollRarity` / `selectReward`) imported from `@game-judge/game-engine`
