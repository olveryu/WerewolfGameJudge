...
- Running tests must not use `| grep` / `| head` / `| tail` to truncate output. Playwright defaults to `--reporter=list` (as in `pnpm run e2e`); the `e2e:core` debug script may use `--reporter=line`.
