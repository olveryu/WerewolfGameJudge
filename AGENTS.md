## WerewolfGameJudge Agent Instructions
...
- `pnpm run e2e:core` — Single-worker debug entry with tracing
- `pnpm run e2e:ui` — Interactive Playwright UI
- `pnpm exec tsc --noEmit` — Type checking
- `pnpm run quality` — typecheck + game-engine build + Knip + agent drift + lint + format + all tests
- `pnpm run knip` — Dead code and dependency detection
- `pnpm run sync:agents` / `pnpm run sync:agents:check` — Generate agent adapters / check drift
