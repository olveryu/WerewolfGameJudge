...
- All user-facing text (alerts, UI labels, error messages shown in-app) must be in Chinese. `showAlert` title uses specific action (e.g., `'创建失败'`), fallback `'请稍后重�?`.
...
Before ending a long session (�? rounds of interaction), briefly review:
