...
| PlayerEngine     | ✅ 完成 | `c03988a` | ~340  | 17    |
| HostEngine       | ✅ 完成 | `c03988a` | ~700  | -     |
...
**目标**: 将 `core/` 中的模块移动到 `v2/` 以匹配 Section 4.1 的目标结构

| 任务                                                      | 状态    | 说明                     |
| --------------------------------------------------------- | ------- | ------------------------ |
| **Domain 层**                                             |         |                          |
| Move `DeathCalculator.ts` → `v2/domain/`                  | ✅ 完成 | 纯函数，无依赖           |
| Move `night/resolvers/` → `v2/domain/resolvers/`          | ✅ 完成 | 18 文件                  |
| Update domain/index.ts                                    | ✅ 完成 | 添加新导出               |
| **Infra 层**                                              |         |                          |
| Move `AuthService.ts` → `v2/infra/Auth.ts`                | ✅ 完成 | Supabase 认证            |
| Move `SimplifiedRoomService.ts` → `v2/infra/Room.ts`      | ✅ 完成 | 房间管理                 |
| Update infra/index.ts                                     | ✅ 完成 | 添加新导出               |
| **Types 层**                                              |         |                          |
| Move `types/GameStateTypes.ts` → `v2/types/GameState.ts`  | ✅ 完成 | 状态类型定义             |
| Move `types/PublicBroadcast.ts` → `v2/types/Broadcast.ts` | ✅ 完成 | 广播类型定义             |
| Create `v2/types/Events.ts`                               | ⏸ 跳过 | 暂不需要，未来可扩展     |
| Update types/index.ts                                     | ✅ 完成 | 统一导出                 |
| **清理**                                                  |         |                          |
| Update all import paths                                   | ✅ 完成 | 全项目 sed 批量替换      |
| Update test mock paths                                    | ✅ 完成 | 4 个测试文件             |
| Update import-boundary.test.ts                            | ✅ 完成 | 新 resolver 路径         |
