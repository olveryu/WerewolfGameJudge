...
你画我猜接龙是第三个正式游戏。每位玩家先自由作画，随后全员在“看画猜词”和“看文字作画”之间交替，最后逐条、逐项同步播放全部接龙结果。
...
| 首棒任务 | 不提供题目，每位玩家直接自由作画                     |
| 绘画时间 | 默认 120 秒                                          |
| 猜词时间 | 默认 15 秒                                           |
| 每棒等待 | 默认 5 秒                                            |
| 结果播放 | 默认每项 3 秒                                        |
| 结果顺序 | 按接龙顺序，每条接龙从第一幅自由画到最后一项逐个展示 |
| 计分     | V1 不计分、不投票、不评选                            |
| 画作形式 | 静态画作，不播放笔画过程                             |
...
$$
D=\left\lceil\frac{N}{2}\right\rceil,\qquad
...
$$
T_{total}=D\times T_d+G\times T_g+N\times T_w+N^2\times T_r
$$
...
- 每条接龙先显示第一幅自由画作，再按顺序显示猜词、画作和占位项。
- 每项揭晓后才显示该项作者头像和名字。
...
| `answering:drawing`    | 首棒自由画布或上一条文字和画布   | “提交画作”             | 已提交人数，不显示画作       |
...
- 底部显示当前接龙的离散时间线：从画作开始与猜词交替，未来项隐藏内容，只显示位置。
...
- 偶数项绘画、奇数项猜词，且第 0 项为自由画作，最终每条长度恰好为 $N$。
...
1. 4 个真实浏览器完成一整局，验证首棒自由作画、后续猜词/画画和逐项结果。
