...
| `answering:text` 首棒  | 出题输入框，不展示上一棒内容     | “提交题目”             | 已提交人数，不显示题目       |
| `answering:drawing`    | 上一条文字和画布                 | “提交画作”             | 已提交人数，不显示画作       |
| `answering:text` 后续  | 上一幅画和猜词输入框             | “提交猜测”             | 已提交人数，不显示猜词       |
...
- 出题阶段不显示上一棒内容，提交按钮和无障碍名称均为“提交题目”。
...
| 客户端状态             | 工作区                                                     | 主操作                                       |
| ---------------------- | ---------------------------------------------------------- | -------------------------------------------- |
| `editing`              | 可编辑输入或画布                                           | 可用条件满足后显示“提交猜词/画作”            |
...
- 底部显示当前接龙的离散时间线：从原始题目开始，画作与猜词交替，未来项隐藏内容，只显示位置。
...
1. 4 个真实浏览器完成 4 轮“文字→绘画”，并逐项验证 32 个结果。
