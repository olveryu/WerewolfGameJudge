...
| **改依赖版本/脚本/CI** | `copilot-instructions.md`          | Project Overview / Tech Stack / 质量命令 / 发版部署流程；确保与 `package.json`、锁文件、`.github/workflows/*.yml`、`scripts/*` 一致 |
|                        | `README.md` / `docs/DEPLOYMENT.md` | 同步开发命令、部署命令、最低 Node 版本；与可执行配置一致                                                                            |
...
3. **权威来源对照**：版本与命令以可执行配置为准。依赖/运行时看 `package.json` 与锁文件；CI/部署看 `.github/workflows/*.yml` 与 `scripts/*`。instruction/README 仅作同步说明，冲突时先修文档。
4. **手动 checklist**（每次发版前/季度）：按上表检查最近变更是否遗漏了文档更新。
