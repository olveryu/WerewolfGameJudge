...
> **状态:设计提案,等待确认后实现。** 所有「承重断言」均经真实代码核验(含 file:line),标 `✓` 为本会话亲自复核的核心事实。
...
- UI:fib **自建薄壳** + compose 既有共享件(`AvatarWithFrame`/`NameStyleText`/`ScreenHeader`),**零狼人 UI 改动**(不抽基类、不碰 690 行 `SeatTile`)。
...
| **Command(命令)**                | `dispatch(actionType, payload)`:动作变成数据对象,engine 当 interpreter 执行                                          | 统一动作入口,DO RPC 面不随游戏数膨胀                                                             |
...
本版改为**参数化抽取**:DO 壳保持 game-agnostic,新增**单一 `dispatch` RPC**(Command)+ 把 `processAction` 的 reduce/normalize 抽成 **`GameEngine` 接口**(Strategy)。增长降为 **O(1) DO 壳 + O(游戏数) 个自包含 engine**。
...
- **不硬抽 UI 基类**:从 690 行 / 58 耦合点的狼人热组件硬抽基类 = wrong abstraction(「duplication 比 wrong abstraction 便宜」)。fib 自建薄壳(§14)。
...
│   + 新增通用:initState(engineType, blob)、dispatch(actionType, payload) │
└───────────────────────────────┬──────────────────────────────┘
...
- **平台壳(不动 + 极小新增)**:`room_state` 单行 JSON blob + `revision`、`#broadcast`、WebSocket hibernation、`getState`/`getRevision`/`cleanup` 已 game-agnostic(✓ 见 §4),**白拿**。仅新增 `initState` + `dispatch` 两个通用方法。
...
- **狼人零回归**:狼人保留全部 bespoke 方法(`assignRoles`/`seat`/`submitAction`/`audioAck`/…)与 `#processAction` → `processAction`,**一行不改**。fib 只走新增 `dispatch`。两条路径同 DO 并存、互不调用,共用 `#broadcast` 与 hibernation。
...
| 4 ✓ | [roomHandlers.ts](../packages/api-worker/src/handlers/roomHandlers.ts) `/create`:D1 insert `rooms` → `if (parsed.initialState) stub.init(blob as GameState)` → 失败回滚 D1   | 建房 = game-agnostic 建房记录 + 游戏专属初始化;`rooms` 表无 gameType 列 → 加 `game_type`;fib 走 engine 的 Factory 构造 |
...
| 8   | transport(`CFRealtimeService`/`ConnectionManager`/`ConnectionFSM`)、`defineGameAction` 完全 game-agnostic(早期核验)                                                          | 直接复用,fib 不重写传输层                                                                                              |
...
dispatch(actionType: string, payload: unknown): Promise<GameActionResult>; // 查 registry → engine → 写 + 广播
```
...
DO 的 `dispatch` 内部(同步,无 `await fetch`):
...
```
POST /fib/update-config → dispatch('UPDATE_CONFIG', { n })        // Lobby-only
POST /fib/sit | /leave  → dispatch('SIT' | 'LEAVE', { ... })
POST /fib/start-round    → 两段式编排(抽词在 Worker,§7):
        1. r1 = dispatch('BEGIN_DRAW', {})            // 相位+坐满守卫 → phase=Starting,广播"出题中"
           !r1.success → return r1                    // fail-fast
        2. word = await generateFibWord(env, { avoid: r1.state.usedWords })   // ★ Worker 侧 await fetch
        3. dispatch('START_ROUND', { word, def, source })   // 同步:随机分角色 + 写词 → phase=Playing
        catch(代码 bug) → dispatch('ABORT_DRAW', {}) + log.error + Sentry + 中文 fail
POST /fib/reveal        → dispatch('REVEAL', {})                  // phase=Revealed
POST /fib/next-round    → 同 start-round 两段式(仅 Revealed)
POST /fib/restart       → dispatch('RESTART', {})                // 任意非 Lobby → Lobby(弃局)
POST /fib/kick          → dispatch('KICK', { targetSeat })       // Lobby-only
POST /fib/clear-seats   → dispatch('CLEAR_SEATS', {})            // Lobby-only
```
...
- **结论**:抽词在**无状态 Worker 路由**(与 `geminiProxy` 同样 `await fetch`),抽到词后传入**同步** DO `dispatch('START_ROUND', { word, ... })`。服务端唯一权威,host 永不见词。
...
- **engine 判别**:`initState` 时把 `engine_type` 存进 DO storage 独立行(**不内联 blob、不碰狼人 `GameState`**)。`dispatch` 读它查 registry。客户端冷启动靠 **D1 `game_type` 列**。`FibState.gameType` 仅自描述。
- **零 settle**:见 §6.4 / §4 #3。
- **狼人零回归**:见 §3。
...
有内置兜底,抽词「必出」;每级降级 `log.warn`(预期,不报 Sentry)。真抛错(代码 bug)→ `dispatch('ABORT_DRAW')` 回 Lobby + `log.error` + `Sentry.captureException` + 中文 `showAlert('开始失败', …)`。不静默吞。
...
- **可共享 vs 必须各自**:可共享 = CRUD 逻辑 + `RosterEntry` + reason codes + UI 范式 + transport;必须各自 = REST 端点、动作分发(fib `dispatch('SIT')` 经 fibEngine vs 狼人 bespoke `seat`)、状态容器(`FibState.seats` vs `GameState.players`)。回归风险 **LOW**(纯函数 + 既有座位测试护栏)。
...
- **`FibFacade`**(`src/services/facade/FibFacade.ts`):**复用** `ConnectionManager`/`CFRealtimeService` transport,`onStateUpdate → fibStore.applySnapshot`。
- **`fibActions.ts`**:复用 `defineGameAction`(只换 `path`/`body`)。
...
interface FibState {
  ...
  seats: Record<number, FibSeat | null>; // 自有最小座位模型
...
- **开局门槛**:坐满 `numberOfPlayers`(4–8);未坐满 `开始本轮` 置灰 + hint;N≥4 保证 1 guesser + 1 honest + ≥2 fibber。
...
- 扩容加 null 空位;缩容仅高位空座可减(否则 fail-fast),不压缩重排。
...
```
首页「创建房间」──▶ GameModePickerSheet(底部弹出,复用 card 视觉)
                     🐺 狼人杀 → BoardPicker→Config→Room(完全不变)
                     🤥 瞎掰王 → FibConfig→/room/create→FibRoom(选人数 4–8)
```
...
房内:  FibRoom·Lobby → host「设置」→ FibConfig({existingRoomCode}) → dispatch('UPDATE_CONFIG') → 回房
```
...
## 13. UI 复用矩阵(自建薄壳 + compose 共享件,零狼人 UI 改动)
...
| 房间 Header                                     | **复用共享** `ScreenHeader.tsx`(已被 10+ 屏用);不碰 `HeaderActions`                                             |
| StatusRibbon / 二维码分享 Modal / JoinRoomModal | **复用**,fib 传精简 props;join 后按 gameType 路由                                                               |
| 座位单元                                        | **不碰 690 行 SeatTile**;自建 `FibSeatCell` = compose `AvatarWithFrame`+`NameStyleText`+座号+大聪明徽章(~30 行) |
| 查看身份卡                                      | **不碰 RoleCardModal**;自建 `FibIdentitySheet`(theme + reanimated)                                              |
| 底部操作面板 / GameModePickerSheet              | fib 自建薄壳 / 新增模式卡                                                                                       |

> **原则**:非平凡且本就共享的低层件直接 compose;**不从狼人热组件硬抽基类**(wrong abstraction)。fib 自建薄壳,零狼人 UI 改动。
...
| 按钮     | 触发                                                                     | 确认                                |
| -------- | ------------------------------------------------------------------------ | ----------------------------------- |
| 设置     | `navigate('FibConfig',{existingRoomCode})` → `dispatch('UPDATE_CONFIG')` | 无                                  |
...
| 文件                                                                        | 变更点                                                                                                                                       | 风险                  |
| --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| `…/schemas/fib.ts`(新)+ `handlers/fibHandlers.ts`(新)+ `index.ts` 挂 `/fib` | 端点 zod + 两段式 start-round 编排(经 `dispatch`)                                                                                            | 中                    |
...
| `src/services/facade/FibFacade.ts`(新)+ `fibActions.ts`(新)                 | 复用 transport + `defineGameAction`                                                                                                          | 中                    |
...
| 文件                                                                          | 变更点                                                   | 风险 |
| ----------------------------------------------------------------------------- | -------------------------------------------------------- | ---- |
| `src/screens/HomeScreen/` + `components/GameModePickerSheet.tsx`(新)          | 创建→模式卡                                              | 低   |
| `src/screens/FibConfigScreen/*`(新)                                           | 人数 stepper(4–8);按 `existingRoomCode` 走 create/update | 低   |
| `src/screens/FibRoomScreen/components/{FibSeatCell,FibIdentitySheet}.tsx`(新) | compose 共享件,零狼人 UI 改动                            | 低   |
| `src/screens/FibRoomScreen/*`(新)                                             | Lobby/Starting/Playing/Revealed                          | 中   |
| `src/screens/FibRulesScreen/*`(新)                                            | 玩法说明(§15)                                            | 低   |
| 各 `__tests__` + `e2e/fibking-happy-path.spec.ts`(新)                         | 见 §18                                                   | 低   |
...
- **worker/DO**(mock `generateFibWord`):`createInitialState` 服务端权威构造;start-round 两段式;双击被 `'BEGIN_DRAW'` 守卫挡;`dispatch('fibking')` 与狼人 bespoke 隔离、**未知 gameType/actionType fail-fast**;**fibEngine 不触达 `#settleIfEnded`**(spy);抽词来源矩阵。
...
- **e2e** `fibking-happy-path.spec.ts`:模式卡建房 → 选人数 → 坐满 → 开始本轮 → 各座查看身份校验过滤 → 公布 → 下一轮。
- **收口**:`pnpm run quality` + fib e2e 全绿。
