...
>
> **一句话**:把 `GameRoom` 这个 Durable Object 从「焊死狼人」改造成「game-agnostic 平台壳 + 按 `gameType` 选择的可插拔游戏 engine」。建房统一按 `gameType + config` 走 create registry;瞎掰王是**第一个**正式注册的 engine;狼人运行期 bespoke action RPC 保留;未来「你画我猜」只需再注册 creator/engine,DO 与核心管线零新增方法。
...
- **不把狼人夜晚业务硬塞进通用基类**:狼人夜晚 action/audio/settle 是热路径且领域复杂,本期不迁入 engine registry。
...
- **狼人运行期零污染**:狼人保留全部 bespoke 方法(`assignRoles`/`seat`/`submitAction`/`audioAck`/…)与 `#processAction` → `processAction`;建房已迁到 `ROOM_CREATE_REGISTRY`。fib 只走新增 `engineAction`。两条路径同 DO 并存、互不调用,共用 `#broadcast` 与 hibernation。
...
| 9 ✓  | `GameStore` 与 `FibStore` 的 revision/listener/applySnapshot/reset/lastAction 已收敛到 `SnapshotStore<TState>`                                                                            | 每个游戏 store 只保留对外能力边界,不再复制 snapshot 管线                                           |
...
> 狼人 bespoke action 方法(`assignRoles`/`seat`/`submitAction`/`audioAck`/…)**一律不动**。
...
**狼人建房也不特殊**:狼人 `/create` 同样 POST `{ gameType:'werewolf', config:{ template } }`,由 `roomCreate/registry.ts` 调 `buildInitialGameState` 服务端构造 blob。请求 schema 使用 `z.strictObject` 只接受 `roomCode/gameType/config`;`initialState` 等多余字段 fail-fast。D1 insert 后必须初始化 DO;DO init 失败则回滚 D1,不会留下未初始化房间。客户端创建成功后只调用 `GameFacade.connectCreatedRoom(roomCode, hostUserId)` 连接并接收服务端 snapshot,不再本地构造权威初始 state。
...
- **game 判别**:`initState` 时把 `game_type` 存进 DO storage 独立行(**不内联 blob、不碰狼人 `GameState`**)。`engineAction` 读它查 registry。客户端冷启动靠 **D1 `game_type` 列**。`FibState.gameType` 仅自描述。
- **零 settle**:见 §6.4 / §4 #3。
- **狼人运行期零污染**:见 §3。
...
| `…/durableObjects/GameRoom.ts`                                                                                  | **仅加 `initState` + `engineAction`** 两个通用方法;不动狼人 bespoke action / `#settleIfEnded`                 | 中;新增不触旧路径            |
...
| `src/services/facade/FibFacade.ts`(新)                                      | 复用 transport runtime;建房走 `IRoomService.createRoom`;动作走 `defineRoomAction`                                                                                   | 中                      |
...
**明确不碰**:`ROLE_SPECS/SCHEMAS/NIGHT_STEPS/RoleId`、狼人 `GameState/gameReducer/normalizeState/#settleIfEnded`/契约测试、`settleGameResults/userStats/XP/gacha`。
...
- **客户端 registry**:当前是一个 `RoomFacadeProvider` 下暴露 `useGameFacade`/`useFibFacade` 两个 accessor。第三个游戏前应抽 `RoomModeAdapter` 或 facade factory registry,避免继续新增 hook。
- **action helper**:`FibFacade` 动作已走 `defineRoomAction`;`defineGameAction`/`apiUtils` 仍绑定狼人 store context。下一步应把 room action caller 提升成所有非狼人游戏复用的模块。
...
- 不碰狼人 `GameState`/`gameReducer`/`normalizeState`/`#settleIfEnded`/夜晚逻辑。
- **不把狼人迁进 engine registry(未来可选)**:狼人保留 bespoke action RPC + `#processAction`;建房已统一进 create registry。待 fib + 你画我猜两个真实 engine 跑通、且重复确实造成维护痛点后,再单独评估运行期回迁;在那之前回迁是零用户价值、最大爆炸半径的重构。
