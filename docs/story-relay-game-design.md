...
- 仅提供「本篇图片」「全部图片」，不复制或分享纯文字。本篇生成一张完整 PNG 长图，全部按故事分别生成图片，包含作者、原始换行和空白记录，不输出空卡牌栏目。截图目标为完整非虚拟化内容，不是回放列表的当前可见区域。
- 微信小程序复用狼人杀的临时图片上传与微信预览流程，全部图片一次打开相册预览；浏览器使用图片文件分享，不支持时下载 PNG。上传或截图失败显示明确错误，不打开残缺相册。
