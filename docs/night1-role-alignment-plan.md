...
> 这部分是本方案的核心升级：**对齐的最终证据必须来自 runtime pipeline**，不能再用 legacy Host 跑出“看似通过”的结果。
...
- **目标协议（硬约束）**：witchAction 是 compound schema，UI/boards 必须提交 `extra.stepResults`
...
- ✅ 必须：UI 提交为 统一协议：`submitAction(actorSeat, { stepResults: { save: <seat|null>, poison: <seat|null> } })`
  - 这符合 schema-first（compound）输入模型
  - 同时更新 UI 测试断言
- ❌ 禁止：为了“兼容旧测试/旧 harness”在 handler 中支持 `{ poison: true }/{ save: true }` 这种 legacy-ish payload。若现有测试仍依赖该形态，应先迁移测试到 wire。
...
> 本节默认你已经理解并会严格执行 **0.1 集成测试门禁**。
> 换句话说：下面每个 commit 都不只是“UI/handler/resolver 对齐”，还必须给出 **current boards/harness 证据**（至少满足 boundary + wire contract + 相关 boards 场景）。
...
- 通过 `src/services/__tests__/boards/boundary.guard.test.ts`
- 通过 `src/services/__tests__/boards/wireProtocol.contract.test.ts`（必须锁死 harness 的 witch payload shape：`target=null` + `extra.stepResults.save/poison`）
- 至少 1 个 boards 集成场景覆盖 witch（例如 `*.integration.test.ts` 中包含 save 与 poison 任一条路径）
...
- 更新/新增 1 个 boards 场景（或扩展现有 board）覆盖 confirm 行为：
...
- 至少 1 个 boards 场景覆盖 wolf vote 写入 `currentNightResults.wolfVotesBySeat`，并能 end night
...
- 至少 1 个 boards 场景覆盖 chooseSeat（建议选 seer/guard 这种最典型的）
...
- boards/harness 不含任何 legacy import（由 boundary guard 证明）
- wireProtocol 契约不只是 schema，还锁死 harness 实际发送 payload（由 wire contract 证明）
- 至少 1 个代表性模板有完整 Night-1 boards 覆盖（从 start night → actions → end night）
...
- 放在：`src/services/engine/handlers/__tests__/actionHandler.contract.test.ts`（或项目已有同类位置）
...
### 5.3 boards harness integration（必须，作为最终证据）

- 放在：`src/services/__tests__/boards/*.integration.test.ts`
- 必须使用 `createHostGame()` + `ctx.runNight(...)` 驱动 Night-1 完整流程
- 至少覆盖：
...
- Handler：`src/services/engine/handlers/actionHandler.ts`
