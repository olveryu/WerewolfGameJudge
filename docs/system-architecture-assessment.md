...
- 浏览器补验收：改用独立 Playwright Chromium，390×844 和 1280×900 截图已检查，真实开发账号藏品可加载；键盘左右切换分类、首次藏品请求 503 后显式报错及重试恢复通过。桌面四列头像放大问题已按现有 `fixed.maxContentWidth` 约束为 600px，手机不变。
...
- 浏览器补验收：本地开发账号实际十连抽返回 200；动画过程中主动查看、关闭结果后再次查看通过。该验证会消耗本地开发账号抽奖券，不涉及生产账号；不代表微信或原生设备验收。

### 第三阶段 3D：启动、账号任务与首页布局

- `useBootProgress` 只组合认证和资源就绪；`useBootAssets` 持有字体、头像请求及释放。保留原有超时和头像首帧等待策略，字体异步完成不再继续已卸载的启动任务，App 继续拥有启动屏关闭。
- 每日奖励移到 AppContent，与首页是否挂载无关；旧账号响应不再给新账号弹奖励提示。未读反馈由按账号分键的共享查询持有，请求消费 AbortSignal，首页只展示并回写已读计数，不保留跨账号本地计数。
- 首页桌面使用受约束的双列任务布局：创建/进入/最近房间为主列，抽奖/游戏内容/公告为次列；手机维持单列任务流和原导航。1280×900 与 390×844 截图已检查，手机无横向溢出。
- 验证：直接以 `/appearance` 深链接启动，未挂载首页仍发起每日奖励请求且返回 200；新增测试覆盖账号切换、退出后重新登录及字体任务取消。`pnpm run quality` 全通过，engine 2,654 + client 10,670 + Worker 296，共 13,620 项。
- 限制：本会话没有 Chrome DevTools 性能追踪工具，未产生冷启动 waterfall、LCP/INP/CLS 或头像阻塞耗时的可靠基线；没有据此声称性能提升，也没有凭猜测移除头像等待。后续性能专项需配置 `chrome-devtools-mcp` 后实测。原生/微信设备与远端 CI 结果仍需单独验收。

第三阶段 3A–3D 的代码修改均已实施；上述性能量化与设备验收不在“已验证完成”的声明内。以下为历史评估正文，当前实现以顶部实施记录及第三阶段记录为准。
