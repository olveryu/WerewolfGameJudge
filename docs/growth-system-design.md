...
房主通过独立的「评选 MVP」入口，在确认线下整局已结束后选择一位开局真人。评选不重开，保留本局详情和复盘。
取消或暂不评选仅关闭弹窗，不消耗机会；确认后本局不可改选，刷新或换设备仍保留结果。「重新开始」为独立按钮，保留分享战报入口。
开局真人名单在开始首夜时冻结，离房不减少人数或取消资格；bot 和旁观者不计入。匿名真人计入人数，但不能获得奖励。
黄金抽次数为 `min(开局真人数 × 2, 30)`；不足 6 名真人可评选但奖励为 0。每局最多奖励一个注册账号一次。
选择绑定局次 nonce，服务端在写入本局 `mvpUserId` 的同一次提交内登记奖励 outbox；不足 6 人也锁定评选结果。
旧局次确认不能作用于新局；重新开始清除本局 MVP，新一局可重新评选。直接重开不发 MVP 奖励。
