...
### 4.3 Host 操作救火重开
1. Host 在屏幕底部找到红色按钮 `🔥 救火重开`（仅游戏进行中显示）
2. 点击后弹出确认对话框：
   - 标题：「救火重开」
   - 内容：「将作废当前局并重新发身份。所有人需要重新查看身份后再开始。是否继续？」
3. 点击「继续重开」确认
4. 系统自动：
   - 清空当前夜晚的所有行动记录
   - **重新洗牌、重新分配角色**
...
### 关于救火重开
- 🚨 **救火重开会重新发身份**，不是恢复原来的身份
- 🚫 **不用于纠错**：如果有人看错身份或操作失误，不应该用救火重开来"修正"
- ✅ **只用于救火**：真正卡死、无法继续时才使用
...
**A:** 
1. 线下口头提醒该玩家操作
2. 如果设备有问题，可以让 Host 使用「救火重开」
3. 如果是故意不操作，建议线下协商处理

### Q3: 想换板子/角色配置怎么做？
**A:** 
1. 游戏未开始时：Host 点击「⚙️ 设置」修改板子
2. 游戏进行中：需要先使用「救火重开」或「重新开始」，然后再修改
