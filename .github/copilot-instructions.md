...
- `@werewolf/api-worker` — Auth API（Cloudflare Worker + D1）
...
- Auth API 由 `packages/api-worker`（Cloudflare Worker）承载，CI `deploy-api-worker` job 自动部署。
- 游戏 API 由 **Supabase Edge Functions** 承载（`game` + `gemini-proxy`）。
- `scripts/deploy.sh` — 仅用于 Supabase Edge Functions 应急手动部署，日常不使用。
