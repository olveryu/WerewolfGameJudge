...
| 层  | 机制                            | 触发                         | 覆盖场景                                      |
| --- | ------------------------------- | ---------------------------- | --------------------------------------------- |
| L0  | `worker: true` heartbeat        | 持续                         | 浏览器后台 tab 节流                           |
| L1  | SDK 自动重连                    | WS 断开 → SDK 检测           | 普通网络闪断、WiFi 切换                       |
| L2  | Status listener ack 重试        | `ConnectionStatus.Live`      | WS 真正断开后恢复（audio-ack 补发）           |
| L3  | Browser `online` event ack 重试 | `window 'online'` 事件       | WS 未断但 HTTP 断了（`setOffline`、DNS 故障） |
| L4  | 前台 DB 拉取                    | `visibilitychange → visible` | 后台期间漏掉的广播                            |
| L5  | Dead Channel Recovery           | `Disconnected` 持续 ≥5s      | SDK rejoin 失败（`CHANNEL_ERROR` 死循环）     |

L0-L1 由 Supabase SDK 内置。L2-L3 互斥（谁先触发清除对方）。L4-L5 独立恢复整体 state。L5 是 SDK 缺陷 workaround（销毁重建 channel）。
