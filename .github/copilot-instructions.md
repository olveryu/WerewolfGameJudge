...
React Native (Expo SDK 55) 狼人杀裁判辅助 app。Supabase 负责房间发现、realtime 传输、state 持久化。支持 iOS / Android / Web。
...
- Gemini 3.1 Flash Lite (AI chat via Edge Function proxy)
...
- `services/infra/` — AudioService / AuthService / RoomService / supabaseClient（Supabase 客户端实例）
...
- `config/` — api / supabase / version / errorMessages / storageKeys / guideContent / emojiTokens / iconTokens 配置（纯配置值，禁止业务逻辑/副作用）
...
- `git push` 自动触发 **Cloudflare Pages**（执行 `scripts/build.sh`）完成前端部署，同时触发 **GitHub CI**（quality + deploy-edge-functions + E2E）。
- 游戏 API 由 **Supabase Edge Functions** 承载，CI `deploy-edge-functions` job 自动部署。
- `scripts/deploy.sh` — 仅用于 Cloudflare Pages 自动部署故障时的应急手动部署，日常不使用。
...
**Transport 层**（CFRealtimeService）：

| 机制                    | 触发              | 覆盖场景                                       |
| ----------------------- | ----------------- | ---------------------------------------------- |
| L1: WS onclose 自动重连 | WS 断开 → onclose | 普通网络闪断、WiFi 切换（指数退避，最多 5 次） |
| Ping/pong keepalive     | 每 30s            | 防止空闲连接被中间件关闭                       |

**协调层**（useConnectionSync）：

| 机制                      | 触发                         | 覆盖场景                                         |
| ------------------------- | ---------------------------- | ------------------------------------------------ |
| L3: Browser `online` 重连 | `window 'online'` 事件       | 网络恢复后立即 reconnectChannel + fetchDB        |
| L4: 前台 DB 拉取          | `visibilitychange → visible` | 后台期间漏掉的广播                               |
| L5: Dead Channel Recovery | `Disconnected` 持续 ≥5s      | L1 重连耗尽后销毁重建 WS（指数退避，最多 10 次） |
| L6: Revision 轮询         | 每 5s（Live + visible）      | 连接存活但广播丢失的兜底                         |

**Facade 层**（ConnectionRecoveryManager）：

| 机制                      | 触发                | 覆盖场景                   |
| ------------------------- | ------------------- | -------------------------- |
| L1: Status listener fetch | 第二次+ `Live` 事件 | WS 重连成功后补拉 DB state |

L1 由 CFRealtimeService 内置。L3-L5 由 useConnectionSync 统一协调。L6 独立轮询兜底。
