...
### 0.6) 修改代码时逐符号验证（MUST follow）

- **任何代码修改（删除、重命名、移动、重构、修改签名）都必须逐个验证受影响的符号**，不可批量推断。
- **禁止批量推断"无影响"**：不可因为"A 和 B 没问题"就推断"同文件的 C 也一定没问题"。每个被修改/删除/重命名的导出符号（函数、类型、常量）必须独立验证。
- **每个受影响符号必须用 `grep_search` 或 `list_code_usages` 确认所有消费者**（区分生产代码和测试代码），逐条检查搜索结果，不可只看结果数量或部分条目就下结论。
- **修改前必须列出每个符号的验证结论**（格式：`符号名 → 消费者: 无 / 有 [列出文件]`），纳入变更计划等待用户确认。
