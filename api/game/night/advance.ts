/**
 * Advance Night API Route — POST /api/game/night/advance
 *
 * 推进夜晚到下一步（Host-only）。
 * 使用 game-engine handleAdvanceNight 纯函数 + gameStateManager 读写广播。
...
import type { AdvanceRequestBody } from '../../_lib/types';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const body = req.body as AdvanceRequestBody;
  ...
  const result = await processGameAction(roomCode, (state: BroadcastGameState) => {
    ...
    return handleAdvanceNight(intent, handlerCtx);
  });
