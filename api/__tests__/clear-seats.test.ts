/**
 * Clear All Seats API Route Tests — POST /api/game/clear-seats
 *
 * 覆盖 405 / 400 / 成功 / 失败。
 */
...
const QUERY = { action: 'clear-seats' };

beforeEach(() => jest.clearAllMocks());

describe('POST /api/game/clear-seats', () => {
  ...
  it('returns 400 on failure', async () => {
    mockProcessGameAction.mockResolvedValue({ success: false, reason: 'host_only' });
    const res = mockResponse();
    await handler(mockRequest({ query: QUERY, body: { roomCode: 'ABCD', hostUid: 'h1' } }), res);
    expect(res._status).toBe(400);
  });
