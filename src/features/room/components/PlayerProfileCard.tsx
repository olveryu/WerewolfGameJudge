...
function resolveAvatarSlot(
  avatarUrl: string | undefined,
  resolveBuiltinAvatarName: (avatarId: string) => string,
): SlotInfo {
  ...
  if (isBuiltinAvatarUrl(avatarUrl)) {
    ...
    return { name: resolveBuiltinAvatarName(id), rarity: getItemRarity(id), typeLabel: '头像' };
  }
...
const PlayerProfileCardComponent: React.FC<PlayerProfileCardProps> = ({ model }) => {
  ...
  const {
    ...
    isLoading: loading,
    isError: error,
  } = useUserProfileQuery(target.userId, {
    enabled: !isBot,
  });

  const handleKick = useCallback(() => {
    if (model.onKick === null) throw new Error('Profile kick action is not available');
    model.onKick();
  }, [model]);

  const handleLeaveSeat = useCallback(() => {
    if (model.onLeaveSeat === null) throw new Error('Profile leave action is not available');
    model.onLeaveSeat();
  }, [model]);

  const handleViewUnlocks = useCallback(() => {
    model.onClose();
    void queryClient.prefetchQuery(userUnlocksOptions(target.userId));
  ...
  return (
    ...
    >
      <View style={styles.card}>
        <CloseButton onPress={model.onClose} />
          ...
          <>
            ...
            <View style={styles.avatarSection}>
              <View style={styles.avatarContainer}>
                <Avatar value={target.userId} size={AVATAR_SIZE} borderRadius={AVATAR_SIZE / 2} />
              </View>
            </View>
        ...
        {profile && !loading && (
          <>
            ...
            {model.isSelf && model.onLeaveSeat !== null && (
            ...
            {!model.isSelf && model.onKick !== null && (
              <PressableScale onPress={handleKick} style={styles.kickButton}>
                <Text style={styles.kickButtonText}>移出座位</Text>
              </PressableScale>
            )}
