/**
 * Werewolf import boundary contract tests.
 *
...
describe('Import Boundary Enforcement', () => {
  ...
  describe('Resolver imports should stay server-authority only', () => {
    // Source of truth is now in @werewolf/game-engine
    const gameEngineResolversDir = path.join(
      workspaceRoot,
      'packages/game-engine/src/werewolf/resolvers',
    );
  ...
  describe('Public API exports validation', () => {
    ...
    const gameEngineSpecRoot = path.join(
      workspaceRoot,
      'packages/game-engine/src/werewolf/models/roles/spec',
    );
