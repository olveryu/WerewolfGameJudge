/**
 * useWerewolfNotepad - notepad state management hook.
 *
...
import { GameStatus } from '@werewolf/game-engine/werewolf/models';
import { ROLE_SPECS, type RoleId } from '@werewolf/game-engine/werewolf/models/roles';
import type { Faction } from '@werewolf/game-engine/werewolf/models/roles/spec/types';
 ...
 *
 * @param facade - WerewolfFacade instance, used to read current game state
 */ export function useWerewolfNotepad(facade: IWerewolfFacade): UseNotepadReturn {
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        // Migrate old single publicNote storage shape to left/right notes.
        const oldPublicNote = (parsed as unknown as Record<string, unknown>).publicNote;
        if (oldPublicNote !== undefined) {
          parsed.publicNoteLeft = oldPublicNote as string;
