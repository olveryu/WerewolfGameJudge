/**
 * useWerewolfActions — unit tests for game control & player night actions hook.
 *
...
import { useWerewolfActions } from '@/hooks/werewolf/useWerewolfActions';
import type { BgmControlState } from '@/hooks/werewolf/useWerewolfBgmControl';
import type { DebugModeState } from '@/hooks/werewolf/useWerewolfDebugMode';
...
type MockFacade = {
  [K in keyof IWerewolfFacade]: jest.Mock;
};
...
type GameActionsDeps = Parameters<typeof useWerewolfActions>[0];
...
interface MockDepsOverrides {
  ...
  >;
  mySeat?: number | null;
  gameState?: Partial<LocalWerewolfState> | null;
}
...
function createDeps(overrides: MockDepsOverrides = {}): GameActionsDeps {
  ...
  return {
    ...
    gameState: null,
    ...rest,
    ...(gameState !== undefined && { gameState: gameState as LocalWerewolfState | null }),
  } as unknown as GameActionsDeps;
}

// ---- Tests ----

describe('useWerewolfActions - game control', () => {
  beforeEach(() => jest.clearAllMocks());

  it('updateTemplate should call facade.updateTemplate', async () => {
    const deps = createDeps();
    const { result } = renderHook(() => useWerewolfActions(deps));
...
describe('useWerewolfActions - player night actions', () => {
  beforeEach(() => jest.clearAllMocks());
...
describe('useWerewolfActions - game state queries', () => {
  beforeEach(() => jest.clearAllMocks());
  ...
  it('getLastNightInfo should return "昨夜平安夜" when lastNightDeaths is null/undefined', () => {
    // Runtime guard for malformed persisted state.
    const deps = createDeps({
      gameState: {
...
describe('useWerewolfActions - handleMutationResult', () => {
  beforeEach(() => jest.clearAllMocks());
