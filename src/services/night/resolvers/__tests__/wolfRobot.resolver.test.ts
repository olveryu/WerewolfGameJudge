...
// =============================================================================
// Type Guard & Assert Contract Tests
// =============================================================================

import { isWolfRobotLearnResult, assertWolfRobotLearnedRoleId } from '@/services/night/resolvers/types';

describe('WolfRobot Type Guards & Asserts', () => {
  describe('isWolfRobotLearnResult', () => {
    it('returns true when learnTarget and learnedRoleId both exist', () => {
      const result = {
        learnTarget: 1,
        learnedRoleId: 'hunter' as RoleId,
      };
      expect(isWolfRobotLearnResult(result)).toBe(true);
    });

    it('returns false when learnedRoleId is missing', () => {
      const result = {
        learnTarget: 1,
        // learnedRoleId missing
      };
      expect(isWolfRobotLearnResult(result)).toBe(false);
    });

    it('returns false when learnTarget is missing', () => {
      const result = {
        learnedRoleId: 'hunter' as RoleId,
        // learnTarget missing
      };
    ...
    it('returns false when result is undefined', () => {
      expect(isWolfRobotLearnResult(undefined)).toBe(false);
    });
  });

  describe('assertWolfRobotLearnedRoleId', () => {
    it('does not throw when learnTarget and learnedRoleId exist', () => {
      const result = {
        learnTarget: 1,
        learnedRoleId: 'hunter' as RoleId,
      };
      expect(() => assertWolfRobotLearnedRoleId(result, 'test')).not.toThrow();
    });

    it('throws TypeError when learnTarget is missing', () => {
      const result = {
        learnedRoleId: 'hunter' as RoleId,
      };
      expect(() => assertWolfRobotLearnedRoleId(result, 'test')).toThrow(TypeError);
      expect(() => assertWolfRobotLearnedRoleId(result, 'test')).toThrow('missing learnTarget');
    });

    it('throws TypeError when learnedRoleId is missing', () => {
      const result = {
        learnTarget: 1,
      };
      expect(() => assertWolfRobotLearnedRoleId(result, 'test')).toThrow(TypeError);
      expect(() => assertWolfRobotLearnedRoleId(result, 'test')).toThrow('missing learnedRoleId');
    });

    it('throws TypeError when result is undefined', () => {
      expect(() => assertWolfRobotLearnedRoleId(undefined, 'test')).toThrow(TypeError);
    });
  });

  describe('resolver always returns learnedRoleId when learning succeeds', () => {
    it('learnedRoleId 合约: 成功学习时必须返回 learnedRoleId', () => {
      const ctx = createContext();
      const input = createInput(2); // villager
      ...
      expect(result.valid).toBe(true);
      expect(result.result?.learnTarget).toBe(2);
      // Contract: when valid + learnTarget exists, learnedRoleId MUST exist
      expect(result.result?.learnedRoleId).toBeDefined();
      expect(isWolfRobotLearnResult(result.result)).toBe(true);
    });
  });
});
