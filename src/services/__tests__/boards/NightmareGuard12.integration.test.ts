...
describe(`${TEMPLATE_NAME} - Host Runtime Integration`, () => {
  ...
  describe('Host Authoritative Gate: 被封锁玩家 action 被拒绝', () => {
    ...
    it('blocked seat 的非空 target 被拒绝（不记录 action，不推进流程，设置 actionRejected）', async () => {
      ctx = await createHostGame(TEMPLATE_NAME, createRoleAssignment());
      ...
      // 7. 验证：actionRejected 被设置（新架构：通过 gameState 广播）
      const actionRejectedAfter = ctx.getState()?.actionRejected;
      // actionRejected 应该从 undefined 变为有值
      expect(actionRejectedAfter).toBeDefined();
      expect(actionRejectedAfter?.action).toBe('submitAction');
      expect(actionRejectedAfter?.reason).toBe(BLOCKED_UI_DEFAULTS.message);
    });
