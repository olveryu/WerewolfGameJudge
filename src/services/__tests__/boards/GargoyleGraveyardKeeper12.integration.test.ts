...
describe(`${TEMPLATE_NAME} - Host Runtime Integration`, () => {
  ...
  describe('Gargoyle 特性', () => {
    ...
    it('石像鬼查验获得目标的具体身份（通过 gameState.gargoyleReveal）', async () => {
      ctx = await createHostGame(TEMPLATE_NAME, createRoleAssignment());
      ...
      // 新架构：通过 gameState.gargoyleReveal 验证
      const gargoyleReveal = ctx.getState()?.gargoyleReveal;
      expect(gargoyleReveal).toBeDefined();
      expect(gargoyleReveal?.targetSeat).toBe(8);
      expect(gargoyleReveal?.result).toBe('预言家'); // seat 8 是预言家
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('石像鬼查验狼人队友：获得狼人身份', async () => {
      ...
      const gargoyleReveal = ctx.getState()?.gargoyleReveal;
      expect(gargoyleReveal).toBeDefined();
      expect(gargoyleReveal?.targetSeat).toBe(4);
      expect(gargoyleReveal?.result).toBe('狼人'); // seat 4 是狼人
    });
