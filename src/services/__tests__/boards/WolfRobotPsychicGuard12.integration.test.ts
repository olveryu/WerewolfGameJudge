 ...
 * - psychic 查死者身份（通过 gameState.psychicReveal）
...
describe(`${TEMPLATE_NAME} - Host Runtime Integration`, () => {
  ...
  describe('Psychic 特性', () => {
    ...
    it('通灵师结果通过 gameState.psychicReveal 广播', async () => {
      const { SCHEMAS } = require('../../../models/roles/spec');
      expect(SCHEMAS.psychicCheck).toBeDefined();
      // psychic 的查验结果通过 gameState.psychicReveal 广播
  ...
  describe('Reveal 验证（新架构）', () => {
    it('机械狼查验触发 gameState.wolfRobotReveal', async () => {
      ctx = await createHostGame(TEMPLATE_NAME, createRoleAssignment());
      ...
      // 新架构：通过 gameState.wolfRobotReveal 验证
      const wolfRobotReveal = ctx.getState()?.wolfRobotReveal;
      expect(wolfRobotReveal).toBeDefined();
      expect(wolfRobotReveal?.targetSeat).toBe(2);
    });

    // NOTE: psychic 查验第一夜无死者，不会触发 reveal
    // 这里测试结构性验证 - psychic 查验会触发 reveal（如果有死者）
    it('通灵师在有死者时触发 gameState.psychicReveal（第一夜验证）', async () => {
      // 第一夜死者：0 号被杀
      ctx = await createHostGame(TEMPLATE_NAME, createRoleAssignment());
      ...
      // 新架构：通过 gameState.psychicReveal 验证
      const psychicReveal = ctx.getState()?.psychicReveal;
      expect(psychicReveal).toBeDefined();
      expect(psychicReveal?.targetSeat).toBe(0);
      expect(psychicReveal?.result).toBe('普通村民'); // 查验到村民（displayName）
    });
  });
});
