...
import type { PlayerJoinAction } from '@werewolf/game-engine/werewolf/reducer/types';
...
type MockGameStateRoomService = IRoomService & {
  getGameState: jest.Mock<Promise<{ state: WerewolfState; revision: number } | null>, [string]>;
};

// Mock RoomService (server snapshot persistence)
const mockRoomService = (stateFactory?: (roomCode: string) => WerewolfState | null) =>
  ({
    getGameState: jest
      .fn<Promise<{ state: WerewolfState; revision: number } | null>, [string]>()
...
/** Helper: build a complete WerewolfState with sensible defaults for tests */
function buildTestState(overrides: Partial<WerewolfState> = {}): WerewolfState {
  return {
    roomCode: 'TEST',
    hostUserId: 'host-uid',
...
const createMockConnectionManager = (
  store?: WerewolfStore,
  roomService?: {
    getGameState(roomCode: string): Promise<{ state: WerewolfState; revision: number } | null>;
  },
) => ({
  connectAndWait: jest
...
describe('WerewolfFacade', () => {
  let facade: WerewolfFacade;
  let testStore: WerewolfStore;
  ...
  beforeEach(() => {
    ...
    testStore = new WerewolfStore();
    const roomService = mockRoomService((roomCode) =>
      buildInitialWerewolfStateFromTemplate(roomCode, 'host-uid', mockTemplate),
    );
  ...
  const fillAllSeatsViaReducer = (
    _facadeInstance: WerewolfFacade,
    template: typeof mockTemplate,
  ) => {
    let state: WerewolfState = testStore.getState()!;
    ...
    for (let i = 0; i < template.numberOfPlayers; i++) {
      ...
      state = werewolfReducer(state, action);
    }
  ...
  describe('Host: takeSeat (HTTP API)', () => {
    ...
    it('should return false when API rejects (invalid seat)', async () => {
      ...
      const result = await facade.takeSeat(999, { displayName: 'Player One' });
  ...
  describe('Host: assignRoles (HTTP API)', () => {
    ...
    it('should return NOT_CONNECTED when no roomCode', async () => {
      ...
      const playerFacade = new WerewolfFacade({
  ...
  describe('markViewedRole (HTTP API)', () => {
    ...
    it('should call view-role API for player (unified HTTP path, no PlayerMessage)', async () => {
      ...
      const playerStore = new WerewolfStore();
  ...
  describe('Host: startNight (HTTP API)', () => {
    ...
    it('should return NOT_CONNECTED when not connected', async () => {
      const emptyFacade = new WerewolfFacade({
        store: new WerewolfStore(),
        connectionManager: mockConnectionManager as unknown as ConnectionManager,
        audioService: mockAudioServiceInstance as unknown as AudioService,
        roomService: mockRoomService(),
  ...
  describe('Host: joinRoom(isHost=true) (DB restore)', () => {
    ...
    it('should restore state from DB and set wasAudioInterrupted=true when ongoing', async () => {
      ...
      const rejoinStore = new WerewolfStore();
      const rejoinRoomService = {
        getGameState: jest.fn().mockResolvedValue(dbState),
      };
      const facadeWithDb = new WerewolfFacade({
        store: rejoinStore,
        connectionManager: createMockConnectionManager(
          rejoinStore,
          rejoinRoomService,
    ...
    it('should set wasAudioInterrupted=false when DB status is not ongoing', async () => {
      ...
      const rejoinStore2 = new WerewolfStore();
    ...
    it('should return no_db_state when no DB state and no template', async () => {
      const emptyStore = new WerewolfStore();
      const emptyRoomService = {
        getGameState: jest.fn().mockResolvedValue(null),
      };
      const facadeWithoutDb = new WerewolfFacade({
        store: emptyStore,
        connectionManager: createMockConnectionManager(
          emptyStore,
          emptyRoomService,
  ...
  describe('postAudioAck retry on reconnect', () => {
    ...
    let retryStore: WerewolfStore;

    const setupRetryFacade = async () => {
      statusListeners = [];
      retryStore = new WerewolfStore();
      ...
      const retryRoomService = {
        getGameState: jest
          .fn<Promise<{ state: WerewolfState; revision: number }>, []>()
          .mockResolvedValue({
