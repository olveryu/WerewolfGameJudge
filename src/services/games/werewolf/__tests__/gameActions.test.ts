...
import type { GameActionsContext } from '@/services/games/werewolf/gameActions';
import {
  assignRoles,
  clearAllSeats,
  clearRevealAcks,
  ...
  submitAction,
  updateTemplate,
} from '@/services/games/werewolf/gameActions';

// ─── Helpers ──────────────────────────────────────────────────────────────────

const DEFAULT_STATE: Partial<WerewolfState> = {
  roomCode: 'ABCD',
  hostUserId: 'host-1',
  players: {
    0: {
      ...
      hasViewedRole: false,
      displayName: 'P0',
    } as unknown as WerewolfState['players'][number],
    1: {
      userId: 'p2',
      seat: 1,
      role: 'seer',
...
function createMockStore(state: Partial<WerewolfState> | null = DEFAULT_STATE): WerewolfStore {
  let currentState = state as WerewolfState | null;
  return {
    getState: jest.fn(() => currentState),
    applySnapshot: jest.fn((s: WerewolfState, _rev: number) => {
      currentState = s;
    }),
  } as unknown as WerewolfStore;
}

function createMockCtx(storeState?: Partial<WerewolfState> | null) {
  return {
