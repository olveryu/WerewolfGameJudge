...
const PHASE_IDLE = 0,
  ...
  PHASE_SETTLING = 2,
  PHASE_WAITING = 3;
const PHASE_GATE_OPEN = 4,
  PHASE_DROPPING = 5,
  PHASE_AUTO_OPEN = 6,
  PHASE_SINGLE_REVEAL = 7;
const PHASE_MULTI_GATE = 8,
  PHASE_MULTI_DROPPING = 9,
  PHASE_MULTI_OPENING = 10,
  PHASE_DONE = 11;
...
interface MachineState {
  ...
  resultRarities: number[];
  droppedIndices: number[];
  multiOpenIdx: number;
  multiOpenTimer: number;
  shellPieces: number[];
  sparkles: number[];
  cancelled: boolean;
...
function constrainToDome(data: number[], i: number, gateOpen: boolean, s: number): void {
  ...
  if (dist > maxR && dist > 0.01) {
    ...
    if (dot > 0) {
      data[base + 2] = rd(data, base + 2) - (1 + DAMPING) * dot * nx;
      data[base + 3] = rd(data, base + 3) - (1 + DAMPING) * dot * ny;
    }
  }
...
function ballCollision(data: number[], a: number, b: number): void {
  const ba = a * STRIDE,
    bb = b * STRIDE;
  const dx = rd(data, bb) - rd(data, ba),
    dy = rd(data, bb + 1) - rd(data, ba + 1);
  const dist = Math.sqrt(dx * dx + dy * dy);
  ...
  const nx = dx / dist,
    ny = dy / dist,
    ov = minD - dist;
  data[ba] = rd(data, ba) - nx * ov * 0.5;
  data[ba + 1] = rd(data, ba + 1) - ny * ov * 0.5;
  data[bb] = rd(data, bb) + nx * ov * 0.5;
  data[bb + 1] = rd(data, bb + 1) + ny * ov * 0.5;
  const rv = rd(data, ba + 2) - rd(data, bb + 2),
    rvy = rd(data, ba + 3) - rd(data, bb + 3);
  const rDot = rv * nx + rvy * ny;
  if (rDot > 0) {
    const imp = rDot * 0.5 * (1 + DAMPING);
    data[ba + 2] = rd(data, ba + 2) - imp * nx;
    data[ba + 3] = rd(data, ba + 3) - imp * ny;
    data[bb + 2] = rd(data, bb + 2) + imp * nx;
    data[bb + 3] = rd(data, bb + 3) + imp * ny;
  }
}
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (by < chuteBot + r && by > holeY2) {
      if (bx < holeCx - chuteHW + r) {
        ...
        data[base + 2] = Math.abs(rd(data, base + 2)) * 0.3;
      }
      if (bx > holeCx + chuteHW - r) {
        data[base] = holeCx + chuteHW - r;
        data[base + 2] = -Math.abs(rd(data, base + 2)) * 0.3;
    ...
    if (by + r > floorY2) {
      ...
      if (Math.abs(rd(data, base + 3)) < 25 * s) {
        data[base + 3] = 0;
        data[base + 2] = 0;
        data[base + 5] = rd(data, base + 5) | F_ON_FLOOR;
      } else {
        data[base + 3] = -rd(data, base + 3) * 0.4;
        data[base + 2] = rd(data, base + 2) * 0.7;
      }
    }
  }
}
...
export default function CapsuleMachineCanvas({
  ...
  useEffect(() => {
    ...
    if (stateRef.current) {
      ...
      stateRef.current.phase = PHASE_IDLE;
    }
  }, [cancelTrigger]);
  ...
  useEffect(() => {
    ...
    function frame(now: number) {
      ...
      if (st.phase !== PHASE_IDLE && st.phase !== PHASE_DONE && !st.cancelled) {
        ...
        if (st.phase === PHASE_TUMBLING) {
          ...
          for (let i = 0; i < NUM_BALLS; i++) {
            ...
            const dx = rd(data, base) - DOME_CX * s,
              dy = rd(data, base + 1) - DOME_CY * s;
            ...
            data[base + 2] =
              ...
              Math.sin(i * 7 + time * 3) * 400 * s * strength * dt;
            data[base + 3] =
              rd(data, base + 3) +
              ty * TUMBLE_FORCE * s * strength * dt +
              Math.cos(i * 5 + time * 2) * 400 * s * strength * dt;
          ...
          if (st.phaseTimer >= TUMBLE_DURATION) {
            st.phase = PHASE_SETTLING;
            st.phaseTimer = 0;
          }
        }
        ...
        if (st.phase === PHASE_SETTLING) {
          for (let i = 0; i < NUM_BALLS; i++) {
            const b = i * STRIDE;
            if (!(rd(data, b + 5) & F_ESCAPED)) {
              data[b + 2] = rd(data, b + 2) * 0.95;
              data[b + 3] = rd(data, b + 3) * 0.95;
            }
          }
        ...
        if (st.phase === PHASE_WAITING) {
          for (let i = 0; i < NUM_BALLS; i++) {
            const b = i * STRIDE;
            if (!(rd(data, b + 5) & F_ESCAPED)) {
              data[b + 2] = rd(data, b + 2) * 0.98;
              data[b + 3] = rd(data, b + 3) * 0.98;
            }
          }
        ...
        if (st.phase === PHASE_GATE_OPEN) {
          ...
          if (found >= 0) {
            ...
            st.capsuleIdx = found;
            st.phase = PHASE_DROPPING;
            st.gateOpen = false;
          }
          if (st.phaseTimer > 2.5 && found < 0) {
            forceBallOut(data, s, BALL_R_SINGLE, st);
            st.phase = PHASE_DROPPING;
            st.gateOpen = false;
          }
        }
        ...
        if (st.phase === PHASE_DROPPING) {
          if (st.capsuleIdx >= 0 && rd(data, st.capsuleIdx * STRIDE + 5) & F_ON_FLOOR) {
            st.phase = PHASE_AUTO_OPEN;
            st.phaseTimer = 0;
            st.autoOpenTimer = 0;
          }
        }
        ...
        if (st.phase === PHASE_AUTO_OPEN) {
          st.autoOpenTimer += dt;
          if (st.autoOpenTimer > 0.5) {
            triggerOpen(st, st.capsuleIdx);
            st.phase = PHASE_SINGLE_REVEAL;
            st.phaseTimer = 0;
            st.revealTimer = 0;
          }
        }

        if (st.phase === PHASE_SINGLE_REVEAL) {
          st.revealTimer += dt;
          if (st.revealTimer > 2.0) st.phase = PHASE_DONE;
        }
        ...
        if (st.phase === PHASE_MULTI_GATE) {
          ...
          if (dc >= st.multiCount) {
            st.gateOpen = false;
            st.phase = PHASE_MULTI_DROPPING;
            st.phaseTimer = 0;
          }
          if (st.phaseTimer > 4) {
            forceMultiOut(data, s, st);
            st.gateOpen = false;
            st.phase = PHASE_MULTI_DROPPING;
            st.phaseTimer = 0;
          }
          if (st.phaseTimer > 0.5 && dc < st.multiCount) {
            for (let i = 0; i < NUM_BALLS; i++) {
              const b = i * STRIDE;
              if (!(rd(data, b + 5) & F_ESCAPED) && !(rd(data, b + 5) & F_ON_FLOOR)) {
                data[b] = rd(data, b) + (HOLE_CX * s - rd(data, b)) * 0.02;
                data[b + 3] = rd(data, b + 3) + 10 * dt * 60;
              }
            }
          }
        }
        ...
        if (st.phase === PHASE_MULTI_DROPPING) {
          let allLanded = true;
          for (const bi of st.droppedIndices) {
            const f = rd(data, bi * STRIDE + 5);
            if (!(f & F_ON_FLOOR) && !(f & F_OPENED)) {
              allLanded = false;
              break;
            }
          }
          if (allLanded) {
            st.phase = PHASE_MULTI_OPENING;
            st.multiOpenIdx = 0;
            st.multiOpenTimer = 0;
            st.phaseTimer = 0;
          }
        ...
        if (st.phase === PHASE_MULTI_OPENING) {
          ...
          if (st.multiOpenIdx >= st.droppedIndices.length && st.multiOpenTimer > 0.6)
            st.phase = PHASE_DONE;
        }
      ...
      if (st.phase !== st.lastPhaseReported) {
        st.lastPhaseReported = st.phase;
      ...
      grd.addColorStop(0, 'rgba(232,232,240,0.35)');
      grd.addColorStop(1, 'rgba(232,232,240,0)');
      ctx!.fillStyle = grd;
      ctx!.fillRect(0, 0, canvasW, canvasH);

      // Floor
      ctx!.fillStyle = MC.floor;
      ctx!.fillRect(0, FLOOR_Y * s, canvasW, canvasH - FLOOR_Y * s);
      ctx!.strokeStyle = MC.floorLine;
      ctx!.lineWidth = 1;
      ctx!.beginPath();
      ctx!.moveTo(30 * s, FLOOR_Y * s);
      ctx!.lineTo(canvasW - 30 * s, FLOOR_Y * s);
      ctx!.stroke();

      // Shadow
      ctx!.fillStyle = MC.shadow;
      ctx!.beginPath();
      ctx!.ellipse(canvasW / 2, BODY_B * s + 24 * s, 148 * s, 14 * s, 0, 0, Math.PI * 2);
      ctx!.fill();
      ...
      ctx!.save();
      ctx!.beginPath();
      ctx!.arc(DOME_CX * s, DOME_CY * s, DOME_R * s, 0, Math.PI * 2);
      ctx!.clip();
      for (let i = 0; i < NUM_BALLS; i++) {
        const base = i * STRIDE,
          flags = rd(data, base + 5);
      ...
      ctx!.beginPath();
      ctx!.ellipse(DOME_CX * s - 55 * s, DOME_CY * s, 16 * s, 72 * s, 0, 0, Math.PI * 2);
      ctx!.fill();

      // Gate
      if (st.gateOpen) {
        ctx!.fillStyle = MC.gate;
        ctx!.fillRect(
      ...
      ctx!.quadraticCurveTo(
        canvasW / 2,
        CHUTE_BOT * s + 12 * s,
        canvasW / 2 + chuteHW - 4 * s,
        CHUTE_BOT * s,
      );
      ctx!.lineTo(canvasW / 2 + chuteHW, CHUTE_TOP * s);
      ctx!.closePath();
      ctx!.fill();
      ctx!.strokeStyle = MC.chuteStroke;
      ctx!.lineWidth = 1.5;
      ctx!.stroke();

      // Dial
      const dialX = canvasW / 2,
        dialY2 = DIAL_Y * s,
        dialR = 32 * s;
      ctx!.fillStyle = MC.dialBody;
      ctx!.beginPath();
      ctx!.arc(dialX, dialY2, dialR, 0, Math.PI * 2);
      ctx!.fill();
      ctx!.strokeStyle = MC.dialStroke;
      ctx!.lineWidth = 2;
      ctx!.stroke();
      ctx!.fillStyle = MC.dialHighlight;
      ctx!.beginPath();
      ctx!.arc(dialX, dialY2 - 23 * s, 8 * s, 0, Math.PI * 2);
      ctx!.fill();
      ctx!.fillStyle = MC.dialKnob;
      ctx!.beginPath();
      ctx!.arc(dialX - 1 * s, dialY2 - 22 * s, 3 * s, 0, Math.PI * 2);
      ctx!.fill();
      // Crosshair
      ctx!.strokeStyle = 'rgba(255,255,255,0.12)';
      ctx!.lineWidth = 2;
      ctx!.beginPath();
      ctx!.moveTo(dialX - 9 * s, dialY2);
      ctx!.lineTo(dialX + 9 * s, dialY2);
      ctx!.stroke();
      ctx!.beginPath();
      ctx!.moveTo(dialX, dialY2 - 9 * s);
      ctx!.lineTo(dialX, dialY2 + 9 * s);
      ctx!.stroke();

      // Base feet
      ctx!.fillStyle = MC.body;
      roundRect(ctx!, BODY_L * s + 18 * s, BODY_B * s, 28 * s, 22 * s, 3 * s);
      ctx!.fill();
      roundRect(ctx!, BODY_R_X * s - 46 * s, BODY_B * s, 28 * s, 22 * s, 3 * s);
      ctx!.fill();
      ...
      for (let i = 0; i < st.shellPieces.length; i += 9) {
        const alpha = rd(st.shellPieces, i + 7);
        if (alpha <= 0) continue;
        ...
        ctx!.arc(
          rd(st.shellPieces, i),
          rd(st.shellPieces, i + 1),
          rd(st.shellPieces, i + 4),
          0,
          Math.PI * 2,
        );
        ctx!.fill();
      ...
      for (let i = 0; i < st.sparkles.length; i += 7) {
        const life = rd(st.sparkles, i + 4);
        if (life <= 0) continue;
        const sx = rd(st.sparkles, i),
          sy = rd(st.sparkles, i + 1);
      ...
      ctx!.fillText(
        drawType === 'golden' ? '\u2605 GOLDEN GACHA \u2605' : '\u2726 GACHA \u2726',
        canvasW / 2,
        (BODY_T + 24) * s,
      );
  ...
  const canvasStyle = { width: canvasW, height: canvasH, display: 'block' as const };
...
function goGate(st: MachineState, data: number[], s: number): void {
  ...
  if (isMulti) {
    ...
    const sorted: number[] = [];
    for (let i = 0; i < NUM_BALLS; i++) {
      if (!(rd(data, i * STRIDE + 5) & F_ESCAPED)) sorted.push(i);
    }
...
function forceBallOut(data: number[], s: number, ballR: number, st: MachineState): void {
  ...
  for (let i = 0; i < NUM_BALLS; i++) {
    const b = i * STRIDE;
    if (!(rd(data, b + 5) & F_ESCAPED) && rd(data, b + 1) > lowY) {
      lowY = rd(data, b + 1);
      low = i;
    }
  }
  if (low >= 0) {
    ...
    data[low * STRIDE + 5] = rd(data, low * STRIDE + 5) | F_ESCAPED;
    data[low * STRIDE + 4] = ballR * s;
    st.capsuleIdx = low;
  }
}
...
function forceMultiOut(data: number[], s: number, st: MachineState): void {
  ...
  while (dc < st.multiCount) {
    ...
    for (let i = 0; i < NUM_BALLS; i++) {
      const b = i * STRIDE;
      if (
        !(rd(data, b + 5) & F_ESCAPED) &&
        !(rd(data, b + 5) & F_ON_FLOOR) &&
        rd(data, b + 1) > lowY
      ) {
        lowY = rd(data, b + 1);
        low = i;
    ...
    if (low < 0) break;
    data[low * STRIDE] = HOLE_CX * s + Math.sin(dc * 7) * 10 * s;
    data[low * STRIDE + 3] = 120 * s;
    data[low * STRIDE + 5] = rd(data, low * STRIDE + 5) | F_ESCAPED;
    data[low * STRIDE + 4] = BALL_R_MULTI * s;
    data[low * STRIDE + 2] = Math.sin(dc * 5) * 100 * s;
    st.droppedIndices.push(low);
    dc++;
  }
  st.droppedCount = dc;
}
...
function triggerOpen(st: MachineState, bi: number): void {
  if (bi < 0) return;
  const data = st.ballData;
  const bx = rd(data, bi * STRIDE),
    by = rd(data, bi * STRIDE + 1),
    br = rd(data, bi * STRIDE + 4);
  ...
  for (let i = 0; i < 10; i++) {
    ...
    st.shellPieces.push(
      ...
      Math.cos(a) * speed,
      Math.sin(a) * speed - 60,
      br * (0.2 + ((i * 37 + 3) % 30) / 100),
      0,
      Math.sin(i * 11) * 5,
      1,
      i < 5 ? -1 : bi % BALL_COLORS.length,
    );
  }
...
function drawBall(
  ...
  ctx.beginPath();
  ctx.arc(bx - r * 0.28, by - r * 0.35, r * 0.22, 0, Math.PI * 2);
  ctx.fill();
  ctx.strokeStyle = 'rgba(255,255,255,0.2)';
  ctx.lineWidth = 1.5;
  ctx.beginPath();
  ctx.arc(bx, by - r * 0.08, r * 0.28, 0, Math.PI * 2);
  ctx.stroke();
