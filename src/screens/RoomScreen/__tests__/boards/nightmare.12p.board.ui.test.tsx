...
import { BLOCKED_UI_DEFAULTS, getSchema } from '@werewolf/game-engine/werewolf/models/roles/spec';
  ...
  describe('actionRejected coverage (CRITICAL: nightmare blocked)', () => {
    ...
    it('blocked seer: tapSeat triggers action → server rejects → shows actionRejected with BLOCKED_UI_DEFAULTS', async () => {
      ...
      reactiveMock.connect((newMock) => {
        ...
        rerender(<RoomScreen route={mockRoomRoute} navigation={mockNavigation} />);
      });
