...
export const SealBreak: React.FC<RoleRevealEffectProps> = ({
  ...
  // ── Crack path Skia objects ──
  const crackSkiaPaths = useMemo(() => {
    return crackSVGs.map((svg) => {
  ...
  return (
    <View style={styles.container} testID={`${testIDPrefix}-container`}>
      ...
      <Animated.View style={[StyleSheet.absoluteFill, canvasContainerStyle]}>
        ...
        {(phase === 'shatter' || shatterProgress.value > 0) && (
          <Canvas style={StyleSheet.absoluteFill}>
            {shards.map((shard) => {
              const endX = cx + Math.cos(shard.angle) * shard.distance;
              const endY = cy + Math.sin(shard.angle) * shard.distance;
              const x = interpolate(shatterProgress.value, [0, 1], [cx, endX]);
              const y = interpolate(shatterProgress.value, [0, 1], [cy, endY]);
              const opacity = interpolate(shatterProgress.value, [0, 0.5, 1], [1, 0.8, 0]);
              return (
                <Group key={`shard-${shard.id}`} opacity={opacity}>
                  <Rect
                    x={x - shard.size / 2}
                    y={y - shard.size / 2}
                    width={shard.size}
