/**
 * ChainShatterLoader — Web CanvasKit 异步加载包装器
 *
 * Web 端 @shopify/react-native-skia 依赖全局 CanvasKit WASM 运行时。
 * 此 Loader 使用 React.lazy + Suspense 确保 ChainShatter 模块仅在
 ...
 */
const ChainShatterFallback: React.FC<RoleRevealEffectProps> = ({ onComplete }) => {
  useEffect(() => {
    onComplete();
  }, [onComplete]);
  return null;
...
const ChainShatterLazy = React.lazy(async () => {
  try {
    if (Platform.OS === 'web') {
      ...
      log.info('[ChainShatter] CanvasKit WASM loaded from CDN');
    }

    const mod = await import('./ChainShatter');
    return { default: mod.ChainShatter };
  } catch (err) {
    log.error('[ChainShatter] Failed to load CanvasKit WASM:', err);
    return { default: ChainShatterFallback };
  }
});
...
/**
 * ChainShatter wrapper — uses React.lazy + Suspense to ensure CanvasKit WASM
 * is loaded before the Skia-dependent ChainShatter module is evaluated on web.
 */
export const ChainShatter: React.FC<RoleRevealEffectProps> = (props) => (
  <Suspense fallback={<LoadingFallback />}>
    <ChainShatterLazy {...props} />
  </Suspense>
);
