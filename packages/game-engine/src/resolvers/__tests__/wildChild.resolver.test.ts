/**
 * WildChild Resolver Unit Tests
 *
 * Tests for wildChildChooseIdolResolver validation and resolution logic.
 */

import type { RoleId } from '@werewolf/game-engine/models/roles';
import type { ActionInput, ResolverContext } from '@werewolf/game-engine/resolvers/types';
import { wildChildChooseIdolResolver } from '@werewolf/game-engine/resolvers/wildChild';
...
describe('wildChildChooseIdolResolver', () => {
  describe('validation', () => {
    it('应该拒绝缺少目标', () => {
      const ctx = createContext();
      const input = createInput(undefined);

      const result = wildChildChooseIdolResolver(ctx, input);
  ...
  describe('nightmare block', () => {
    ...
    // WildChild resolver only handles the special case: blocked + skip is allowed (even though canSkip=false).
