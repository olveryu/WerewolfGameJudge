...
export const wildChildChooseIdolResolver: ResolverFn = (context, input) => {
  const { actorSeat, players, currentNightResults } = context;
  const target = input.target;

  // WildChild normally must choose someone (canSkip=false)
  ...
  const schema = SCHEMAS.wildChildChooseIdol;
