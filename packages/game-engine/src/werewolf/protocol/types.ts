/**
 * Werewolf protocol type definitions.
 *
 * Single Source of Truth for all wire protocol types.
 * Other files must import werewolf wire types from here.
 *
 ...
 *
 * Produced during server-side inline progression, written to `WerewolfState.pendingAudioEffects`.
...
/**
 * RosterEntry is game-agnostic and lives in `protocol/common.ts` so non-werewolf engines
 * can import it without transitively pulling in RoleId/Team. Werewolf protocol re-exports
 * it because `WerewolfState.roster` uses the same shared display record.
 */
...
// =============================================================================
// WerewolfState — single authoritative state type
// =============================================================================
...
export interface WerewolfState {
  ...
  // --- Execution state ---
  /** Night-1 action records (normalizeWerewolfState guarantees non-undefined). Cleared on endNight. */
  ...
  /** Pending reveal ack userId[] (normalizeWerewolfState guarantees non-undefined). Cleared and progresses after all ack. */
   ...
   *
   * Notes:
   * - Part of WerewolfState (publicly broadcast), but UI generally doesn't depend on it;
   *   UI only renders from schema + WerewolfState, filtered by myRole.
   ...
   * - UI: renders bottom buttons solely from schema + WerewolfState, no local state machine derivation allowed.
   */
