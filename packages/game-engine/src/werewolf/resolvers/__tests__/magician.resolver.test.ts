...
import { magicianSwapResolver } from '@werewolf/game-engine/werewolf/resolvers/magician';
import type { ActionInput, ResolverContext } from '@werewolf/game-engine/werewolf/resolvers/types';
...
describe('magicianSwapResolver', () => {
  ...
  describe('nightmare block', () => {
    ...
    it('被噩梦之影封锁时 resolver 不再拒绝（由 handler 层统一处理）', () => {
      const ctx = createContext({
