...
// Public handler helpers consumed by tests and board integration harnesses.
...
export function handleSubmitAction(
  ...
  // authoritative override using the full WerewolfState here (shares logic with confirmContext for hunter/wolfKing).
  ...
  if (schema.kind === 'wolfVote' && handlerResult.kind === 'success') {
    ...
    for (const action of handlerResult.actions) {
      tempState = werewolfReducer(tempState, action);
    }
