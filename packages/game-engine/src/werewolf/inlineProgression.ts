...
import type { AudioEffect, WerewolfState } from './protocol/types';
 ...
 */
function isStepComplete(state: WerewolfState): boolean {
 ...
 */
function isUnchosenBottomCardStep(state: WerewolfState): boolean {
...
export function runInlineProgression(
  ...
  for (let i = 0; i < MAX_PROGRESSION_LOOPS; i++) {
    ...
    if (decision === 'advance') {
      ...
      for (const action of result.actions) {
        currentState = werewolfReducer(currentState, action);
      }
  ...
  if (
    ...
    currentState = werewolfReducer(currentState, setDeadlineAction);
  ...
  if (allAudioEffects.length > 0) {
    ...
    currentState = werewolfReducer(currentState, setEffectsAction);
    currentState = werewolfReducer(currentState, setAudioPlayingAction);
