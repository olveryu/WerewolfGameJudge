...
import { WerewolfStore } from '@werewolf/game-engine/werewolf/store/WerewolfStore';
...
describe('WerewolfStore', () => {
  ...
  describe('setState()', () => {
    it('should update state and increment revision', () => {
      ...
      const newState = createMinimalState({ status: GameStatus.Seated });
      store.setState(newState);

      expect(store.getState()?.status).toBe(GameStatus.Seated);
    ...
    it('should notify listeners on each setState', () => {
      ...
      store.initialize(createMinimalState());
      store.setState(createMinimalState({ status: GameStatus.Seated }));
  ...
  describe('updateState()', () => {
    it('should apply updater function', () => {
      ...
      expect(store.getState()?.status).toBe(GameStatus.Assigned);
    });
    ...
    it('should increment revision', () => {
      ...
      store.updateState((state) => ({ ...state, status: GameStatus.Seated }));
  ...
  describe('applySnapshot()', () => {
    it('should apply snapshot with higher revision', () => {
      store.initialize(createMinimalState());
      const newState = createMinimalState({ status: GameStatus.Ongoing });
    ...
    it('should ignore snapshot with lower revision', () => {
      store.initialize(createMinimalState());
      store.setState(createMinimalState({ status: GameStatus.Seated })); // revision = 2
      store.setState(createMinimalState({ status: GameStatus.Assigned })); // revision = 3

      const oldState = createMinimalState({ status: GameStatus.Unseated });
    ...
    it('should ignore snapshot with equal revision', () => {
      store.initialize(createMinimalState({ status: GameStatus.Seated }));
      const oldState = createMinimalState({ status: GameStatus.Unseated });
