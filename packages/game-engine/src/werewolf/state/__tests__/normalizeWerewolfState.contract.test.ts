/**
 * normalize.contract.test.ts - normalizeWerewolfState contract tests
 *
 * Ensures normalizeWerewolfState correctly passes through all fields of WerewolfState.
 * When adding new fields, if you forget to pass them through in normalizeWerewolfState, this test will fail.
 */
...
/**
 * List of all top-level fields of WerewolfState (single source of truth)
 *
 * When adding new fields to WerewolfState:
 * 1. Add the field name to this list
 * 2. Add the pass-through in normalizeWerewolfState
 * 3. Run this test to verify
 */
const GAME_STATE_FIELDS: (keyof WerewolfState)[] = [
...
describe('normalizeWerewolfState contract', () => {
  /**
   * Create a full WerewolfState containing all fields
   */
  const createFullState = (): WerewolfState => {
    return {
      // Core required fields
      roomCode: 'TEST',
      hostUserId: 'host-uid',
  ...
  it('should preserve all WerewolfState fields after normalization', () => {
    const fullState = createFullState();
    const normalized = normalizeWerewolfState(fullState);
  ...
  it('should not drop any fields during normalization (snapshot)', () => {
    ...
    const normalizedFields = Object.keys(normalized) as (keyof WerewolfState)[];
  ...
  it('should preserve undefined optional fields as undefined', () => {
    ...
    const normalized = normalizeWerewolfState(minimalState);
