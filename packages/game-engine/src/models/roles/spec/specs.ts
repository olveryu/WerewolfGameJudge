...
export const ROLE_SPECS = {
  ...
  maskedMan: {
    ...
    team: Team.Good,
    description:
      '负伤免死；首次在夜晚受到致命伤害（狼人袭击/毒药/摄梦连带等）时转为负伤状态而非出局；进入负伤状态后，于下一次白天发言结束后强制出局',
    structuredDescription: {
      passive: '负伤免死',
      trigger: '首次在夜晚受到致命伤害（狼人袭击/毒药/摄梦连带等）时转为负伤状态而非出局',
      restriction: '进入负伤状态后，于下一次白天发言结束后强制出局',
    },
    tags: ['immune'],
    abilities: [],
    nightSteps: [],
  ...
  treasureMaster: {
    ...
    team: Team.Third,
    description:
      '永久狼人阵营，不与狼队见面、不共享信息；首夜查看3张底牌，每晚从神职牌和平民牌中选1张获得完整技能；不可连续两晚选同一张；被查验时以当夜选择身份为准；狼队全灭后可主导袭击；底牌固定为1张普通狼人牌+1张神职牌+1张平民牌（不含技能狼）；所有底牌技能仅当夜生效，未触发则失效；狼队全灭后选蒙面人，白天被公投出局且负伤免死未触发过，可触发免死并于当夜袭击2名不同玩家，袭击结束后立即出局',
    structuredDescription: {
      skill: '首夜查看3张底牌，每晚从神职牌和平民牌中选1张获得该角色完整技能',
      passive:
        '永久狼人阵营；不与狼队见面、不共享信息；被查验时以当夜选择的身份为准；狼队全灭后可主导袭击',
      restriction:
        '不可连续两晚选同一张；底牌固定为1张普通狼人牌+1张神职牌+1张平民牌（不含技能狼）；所有底牌技能仅当夜生效，未触发则失效',
      trigger:
        '狼队全灭后选蒙面人，白天被公投出局且负伤免死未触发过，可触发免死并于当夜袭击2名不同玩家，袭击结束后立即出局',
    },
    tags: ['transform'],
    abilities: [
      {
