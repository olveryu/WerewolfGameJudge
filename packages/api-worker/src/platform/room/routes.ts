...
export function createRoomRoutes(
  ...
  requireAuthentication: MiddlewareHandler<AppEnv>,
): Hono<AppEnv> {
  const roomRoutes = new Hono<AppEnv>();

  roomRoutes.post('/create', requireAuthentication, jsonBody(createRoomSchema), async (c) => {
    const hostUserId = c.var.userId;
    const input = c.req.valid('json');
    const module = resolveGameModule(input.gameType);
  ...
  roomRoutes.post('/command', requireAuthentication, jsonBody(roomCommandSchema), async (c) => {
    const input = c.req.valid('json');
  ...
  roomRoutes.post('/delete', requireAuthentication, jsonBody(roomLocatorBodySchema), async (c) => {
    ...
    if (existing.status === 'active') {
      const authorization = await callDurableObject(() =>
